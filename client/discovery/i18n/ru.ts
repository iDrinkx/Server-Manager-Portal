...
Object.assign(ru, { requestsAdmin: { page: { internalFetchHint: 'Если используется общедоступный URL через обратный прокси, контейнер портала может не иметь к нему доступа. Добавьте внутренний URL для загрузки в Настройки → Интеграции (имя службы Docker или IP-адрес LAN, например: http://jellyseerr:5055).' } } });

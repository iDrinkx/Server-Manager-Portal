...
Object.assign(ru, { requestsAdmin: { page: { pendingSummary: '{count} ожидают · полная проверка в стиле Seerr с профилями, папками, тегами и сезонами', reviewQueue: 'Очередь проверки', description: 'Одобряйте или отклоняйте запросы портала, обрабатывайте обращения и управляйте списком блокировки.', issues: 'Проблемы', blocklist: 'Список блокировки', configuredConnectionFailed: 'Приложение запросов настроено, но подключиться не удалось (см. ниже)', connectHint: 'Подключите Seerr, Overseerr или Jellyseerr в Настройки → Интеграции, чтобы управлять запросами здесь.', notConfigured: 'Приложение запросов не настроено', notConfiguredHint: 'Укажите тип приложения запросов, URL и API-ключ в Настройки → Интеграции. Ombi пока не поддерживает одобрение в портале.', internalFetchHint: 'Если используется общедоступный URL через обратный прокси, контейнер портала может не иметь к нему доступа. Добавьте внутренний URL для загрузки в Настройки → Интеграции (имя службы Docker или IP-адрес LAN, например: http://jellyseerr:5055).', notSupported: 'Тип приложения запросов не поддерживается', notSupportedHint: 'Одобрение в портале работает с Seerr, Overseerr и Jellyseerr. Для Ombi требуется внешний интерфейс запросов.' }, filters: { allRequesters: 'Все заявители', pending: 'Ожидают', processing: 'В обработке', available: 'Доступно', failed: 'Ошибка', approved: 'Одобрено', declined: 'Отклонено', search: 'Поиск по названию или заявителю...', allTypes: 'Все типы', allQuality: 'Все качества', hdOnly: 'Только HD', '4kOnly': 'Только 4K', allDates: 'Все даты', last7Days: 'Последние 7 дней', last30Days: 'Последние 30 дней' }, labels: { requestedBy: 'Запрошено', updated: 'Обновлено', lastActionBy: 'Последнее действие от', seasons: 'Сезоны', status: 'Статус', created: 'Создано', reason: 'Причина', internalFetchUrl: 'Внутренний URL загрузки' }, empty: { noStatusRequests: 'Нет запросов со статусом «{status}»', noFilteredRequests: 'Нет запросов, соответствующих фильтрам', allCaughtUp: 'Всё обработано.' } } });

...
Object.assign(ja, { requestsAdmin: { page: { pendingSummary: '{count} 件保留中 · プロファイル、フォルダー、タグ、シーズンを含む Seerr 形式の完全な確認', reviewQueue: '審査キュー', description: 'ポータルのリクエストを承認または却下し、問題を整理してブロックリストを管理します。', issues: '問題', blocklist: 'ブロックリスト', configuredConnectionFailed: 'リクエストアプリは設定されていますが、接続に失敗しました（以下を参照）', connectHint: '設定 → 連携 で Seerr、Overseerr、または Jellyseerr を接続すると、ここでリクエストを管理できます。', notConfigured: 'リクエストアプリが未設定です', notConfiguredHint: '設定 → 連携 でリクエストアプリの種類、URL、API キーを設定してください。Ombi はポータル内での承認にまだ対応していません。', internalFetchHint: '公開リバースプロキシ URL を使用している場合、ポータルのコンテナから到達できないことがあります。設定 → 連携 で内部取得 URL を追加してください（Docker サービス名または LAN IP。例: http://jellyseerr:5055）。', notSupported: 'この種類のリクエストアプリには対応していません', notSupportedHint: 'ポータル内での承認は Seerr、Overseerr、Jellyseerr で利用できます。Ombi では外部のリクエスト UI を使用してください。' }, filters: { allRequesters: 'すべてのリクエスト者', pending: '保留', processing: '処理中', available: '利用可能', failed: '失敗', approved: '承認済み', declined: '却下済み', search: 'タイトルまたはリクエスト者を検索...', allTypes: 'すべての種類', allQuality: 'すべての品質', hdOnly: 'HD のみ', '4kOnly': '4K のみ', allDates: 'すべての日付', last7Days: '過去 7 日間', last30Days: '過去 30 日間' }, labels: { requestedBy: 'リクエスト者', updated: '更新', lastActionBy: '最終操作者', seasons: 'シーズン', status: 'ステータス', created: '作成日', reason: '理由', internalFetchUrl: '内部取得 URL' }, empty: { noStatusRequests: '{status} のリクエストはありません', noFilteredRequests: 'フィルターに一致するリクエストはありません', allCaughtUp: 'すべて対応済みです。' } } });
